...
fun JavaClassName.toKPoet(): KotlinClassName {
    ...
    return KotlinClassName(packageName, simpleNames.first(), *simpleNames.drop(1).toTypedArray())
}

/**
 * Some classes, like List or Byte have the same class name but a different package for their kotlin
 * equivalent.
 */
private fun JavaClassName.getPackageNameInKotlin(): String {
    if (
        packageName() in listOf(javaUtilPkg, javaLangPkg, kotlinJvmFunction) &&
            simpleNames().size == 1
    ) {
...
fun JavaTypeVariableName.toKPoet() =
    KotlinTypeVariableName.invoke(if (name == "?") "*" else name, *bounds.toKPoet().toTypedArray())
...
fun JavaParameterSpec.toKPoet(): KotlinParameterSpec {
    ...
    return KotlinParameterSpec.builder(
            paramName,
            // Not using built in javapoet interop because of bug
            // https://github.com/square/kotlinpoet/issues/1181
