...
internal class BaseModelAttributeInfo(
    ...
    init {
        ...
        hasFinalModifier =
            if (memoizer.environment.backend == XProcessingEnv.Backend.KSP) {
    ...
    private fun validateAnnotationOptions(
        ...
        if (
            options.contains(EpoxyAttribute.Option.IgnoreRequireHashCode) &&
                options.contains(EpoxyAttribute.Option.DoNotHash)
        ) {
            logger.logError(
    ...
    private fun findGetterAndSetterForPrivateField(logger: Logger) {
        classElement.getDeclaredMethods().forEach { method ->
            ...
            if (
                (methodName == String.format("get%s", capitalizeFirstLetter(fieldName)) ||
                    methodName == String.format("is%s", capitalizeFirstLetter(fieldName)) ||
            ...
            if (
                (methodName == String.format("set%s", capitalizeFirstLetter(fieldName)) ||
                    startsWithIs(fieldName) &&
                        methodName ==
                            String.format("set%s", fieldName.substring(2, fieldName.length))) &&
                    !method.isPrivate() &&
                    !method.isStatic() &&
                    parameters.size == 1
            ) {
     ...
     * and getter method. Setter and getter annotations are stored separately since the annotation
     * may not target both method and parameter types.
     */
    private fun buildAnnotationLists(attribute: XFieldElement, annotations: List<XAnnotation>) {
        ...
        if (
            memoizer.environment.backend == XProcessingEnv.Backend.KSP &&
                attribute.declaration.isKotlinOrigin()
        ) {
            if (!attribute.type.typeName.isPrimitive) {

                // Look at just simple name of annotation as there are many packages providing them
                // (eg androidx, jetbrains)
                val annotationSimpleNames =
