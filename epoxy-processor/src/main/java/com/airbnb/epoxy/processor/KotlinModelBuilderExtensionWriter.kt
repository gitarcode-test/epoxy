...
internal class KotlinModelBuilderExtensionWriter(val filer: XFiler, asyncable: Asyncable) :
    ...
    fun generateExtensionsForModels(
        ...
        generatedModels
            ...
            .mapNotNull("generateExtensionsForModels") { packageName, models ->
                buildExtensionFile(packageName, models, processorName)
            }
    ...
    private fun buildExtensionFile(
        ...
        models
            .map {
                if (it.constructors.isEmpty()) {
                    ...
                    it.constructors.map { constructor -> buildExtensionsForModel(it, constructor) }
                }
            }
            .flatten()
        ...
        // We suppress Deprecation warnings for this class in case any of the models used are
        // deprecated.
    ...
    private fun buildExtensionsForModel(
        ...
        FunSpec.builder(getMethodName(model)).run {
            ...
            if (modelClass is ParameterizedTypeName) {
                ...
                modelClass.typeArguments.filterIsInstance<TypeVariableName>().let {
                    if (it.isNotEmpty()) addTypeVariables(it)
                }
            }
            ...
            addStatement("add(")
            beginControlFlow("%T(${params.joinToString(", ") { it.name }}).apply", modelClass)
            addStatement("modelInitializer()")
            endControlFlow()
            addStatement(")")

            model.originatingElements().forEach { addOriginatingElement(it) }
            return build()
        }
    }
