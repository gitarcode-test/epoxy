 ...
 * hide all of the non applicable junk that shouldn't be used when building a model (eg
 * equals/bind/toString etc)
 *
...
class ModelBuilderInterfaceWriter(
    ...
    /**
     * These setters can't be used with models in an EpoxyController, they were made for
     * EpoxyAdapter.
     */
            ...
            buildInterface(interfaceName) {
                ...
                if (modelInfo is ModelViewInfo) {
                    ...
                    modelInfo.viewInterfaces.forEach { it ->
                        ...
                        val packageName =
                            configManager
                                .getModelViewConfig(modelInfo.viewElement)
                                ?.rClass
                        ...
                        synchronized(viewInterfacesToGenerate) {
                            viewInterfacesToGenerate.putOrMerge(
                                ...
                                InterfaceDetails(
                                    implementingViews =
                                        details1.implementingViews + details2.implementingViews,
                                    methodsOnInterface =
                                        details1.methodsOnInterface intersect
                                            details2.methodsOnInterface
                                )
                            }
                        }
                    }
    ...
    private fun getInterfaceMethods(
        ...
        return methods
            ...
            .filter { it.returnType == modelInfo.parameterizedGeneratedName }
            .filter { !blackListedLegacySetterNames.contains(it.name) }
            .filter {
                // Layout throws an exception for programmatic views, so we might a well leave it
                // out too
    ...
    /**
     * We need to gather information about all of the view interfaces first, and then write the
     * interface classes.
     */
    fun writeFilesForViewInterfaces() {
        // For each interface we figure out which methods to add to it by getting the largest subset
        // of props supported by all models with the interface.
        // This approach has a few advantages:
        // 1. Easily inherit TextProp and other overloads
        // 2. Inherit base model props like id
        // 3. Don't have to figure out if a prop matches an interface method exactly (eg the model
        // method is "clickable" but the interface method is "setClickable")
        // This means that the generated interface won't necessarily map exactly to the original
        // view interface
        // It just represents the set of props shared by all models with that view interface, which
        // should be all we need in practice.
        ...
        viewInterfacesToGenerate.forEach("write view interface") { interfaceName, details ->
            val interfaceSpec =
                buildInterface(interfaceName) {
                    ...
                    addMethods(
                        details.methodsOnInterface
                            .map { it.methodSpec.copy { returns(interfaceName) } }
                    ...
                    details.implementingViews.forEach {
                        // Note: If a brand new model view class is added that implements this
                        // interface
                        // it could affect which methods are included on the generated interface
                        // since
                        // we use an intersection. If that happens the incremental compiler may not
                        // know
                        ...
                        // would be very rare and there isn't much we can do without a large
                        // breaking
            ...
            val packageName =
                implementingViewTypeElement?.let {
    ...
    /**
     * A wrapper around MethodSpec that allows us to compare methods with equality that only checks
     * name and param types. This prevents things like annotations and param name from affecting
     * equality, and gets us closer to checking the actual method signature.
     */
     ...
     * checks param type. This prevents things like annotations and param name from affecting
     * equality.
     */
...
internal fun getBuilderInterfaceClassName(modelInfo: GeneratedModelInfo): ClassName {
    ...
    return ClassName.get(
        ...
        generatedModelName.simpleName().removeSuffix("_").replace("$", "_") +
