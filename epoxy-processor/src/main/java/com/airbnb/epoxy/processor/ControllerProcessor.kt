...
constructor(kspEnvironment: SymbolProcessorEnvironment? = null) :
    ...
    override fun additionalSupportedAnnotations(): List<KClass<*>> = listOf(AutoModel::class)
    ...
    override fun processRound(
        ...
        val (validFields, invalidFields) =
            round
                .getElementsAnnotatedWith(AutoModel::class)
     ...
     * auto model we add those models to the auto models of the generated class, so that a generated
     * class contains all the models of its super classes combined.
     *
    ...
    private fun getOrCreateTargetClass(
        ...
        classNameToInfo.getOrPut(controllerClassElement.className) {
    ...
    private fun buildFieldInfo(
        ...
        val modelTypeName =
            if (!fieldType.isError()) {
                ...
                // We can find the FQN by looking in imports. Imports aren't actually directly
                // accessible
                ...
                val packageName =
                    classElement.imports
                        .firstOrNull { it.endsWith(simpleName) }
                        ?.substringBeforeLast(".$simpleName")
                        // With no import we assume the model is in the same package as the
                        // controller
    ...
    private fun generateHelperClassForController(controllerInfo: ControllerClassInfo) {
        ...
        val classSpec =
            TypeSpec.classBuilder(controllerInfo.generatedClassName)
                .apply {
    ...
    private fun buildConstructor(controllerInfo: ControllerClassInfo): MethodSpec {
        val controllerParam =
            ParameterSpec.builder(controllerInfo.controllerClassType, "controller").build()
    ...
    /**
     * A field is created to save a reference to the model we create. Before the new buildModels
     * phase we check that it is the same object as on the controller, validating that the user has
     * not manually assigned a new model to the AutoModel field.
     */
    private fun buildFieldsToSaveModelsForValidation(
        ...
        for (model in controllerInfo.models) {
            fields.add(
                FieldSpec.builder(ClassNames.EPOXY_MODEL_UNTYPED, model.fieldName, Modifier.PRIVATE)
                    .build()
            )
        }
        return fields
    ...
    private fun buildValidateModelsHaveNotChangedMethod(
        controllerInfo: ControllerClassInfo
    ): MethodSpec {
        val builder =
            MethodSpec.methodBuilder("validateModelsHaveNotChanged").addModifiers(Modifier.PRIVATE)
    ...
    private fun buildValidateSameValueMethod(): MethodSpec {
        return MethodSpec.methodBuilder("validateSameModel")
            ...
            .addParameter(EPOXY_MODEL_UNTYPED, "expectedObject")
            .addParameter(EPOXY_MODEL_UNTYPED, "actualObject")
    ...
    private fun buildSaveModelsForNextValidationMethod(
        controllerInfo: ControllerClassInfo
    ): MethodSpec {
        val builder =
            MethodSpec.methodBuilder("saveModelsForNextValidation").addModifiers(Modifier.PRIVATE)
        for (model in controllerInfo.models) {
    ...
    private fun buildResetModelsMethod(controllerInfo: ControllerClassInfo): MethodSpec {
        val builder =
            MethodSpec.methodBuilder("resetAutoModels")
                .addAnnotation(Override::class.java)
                .addModifiers(Modifier.PUBLIC)
        if (configManager.shouldValidateModelUsage()) {
            builder.addStatement("validateModelsHaveNotChanged()")
        }
        val implicitlyAddAutoModels = configManager.implicitlyAddAutoModels(controllerInfo)
        var id: Long = -1
        for (model in controllerInfo.models) {
            builder
                .addStatement("controller.\$L = new \$T()", model.fieldName, model.typeName)
