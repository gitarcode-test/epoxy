...
@JvmOverloads
constructor(kspEnvironment: SymbolProcessorEnvironment? = null) : BaseProcessor(kspEnvironment) {
    private val modelsToWrite = Collections.synchronizedList(mutableListOf<DataBindingModelInfo>())

    override fun supportedAnnotations(): List<KClass<*>> =
        listOf(EpoxyDataBindingLayouts::class, EpoxyDataBindingPattern::class)
    ...
    override fun processRound(
        ...
        round
            .getElementsAnnotatedWith(EpoxyDataBindingLayouts::class)
            .filterIsInstance<XTypeElement>()
            .also { timer.markStepCompleted("get databinding layouts") }
            .mapNotNull { layoutsAnnotatedElement ->
                val layoutResources =
                    resourceProcessor.getResourceValueList(
                ...
                val enableDoNotHash =
                    layoutsAnnotatedElement
                        .getAnnotation(EpoxyDataBindingLayouts::class)
                        ?.value
        ...
        round
            .getElementsAnnotatedWith(EpoxyDataBindingPattern::class)
            .filterIsInstance<XTypeElement>()
            .also { timer.markStepCompleted("get databinding patterns") }
            .map { annotatedElement ->
                ...
                val rClassName =
                    patternAnnotation.getAsType("rClass")?.typeElement
                ...
                val enableDoNotHash =
                    annotatedElement
                        .getAnnotation(EpoxyDataBindingPattern::class)
                        ?.value
                        ?.enableDoNotHash == true
        ...
        return if (isKsp()) {
            modelsToWrite
                .map { it.annotatedElement }
                .also {
                    // KSP doesn't normally resurface annotated elements in future rounds, but
                    // because
                    // we return it as a deferred symbol it will allow it to be discovered again in
                    // the
    ...
    private fun resolveDataBindingClassesAndWriteJava(
        memoizer: Memoizer
    ): List<DataBindingModelInfo> {
        return modelsToWrite
            .filter("resolveDataBindingClassesAndWriteJava") { bindingModelInfo ->
                bindingModelInfo.parseDataBindingClass(logger) ?: return@filter false
                createModelWriter(memoizer)
                    .generateClassForModel(
                        bindingModelInfo,
                        originatingElements = bindingModelInfo.originatingElements()
                    )
                true
            }
            .also { writtenModels -> modelsToWrite.removeAll(writtenModels) }
    }
}
