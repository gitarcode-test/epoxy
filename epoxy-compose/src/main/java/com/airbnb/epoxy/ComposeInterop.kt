...
/**
 * An epoxy viewModel that can inflate a Composable function The keys parameter is responsible for
 * recomposition of the composable function in epoxy. Make sure the class of the object passed in as
 * keys, have equals implemented, that is their equality can be checked.
 *
 * However if your composeFunction relies on mutableState to describe your UI, then passing in key
 * as parameter is not needed.
 *
...
class ComposeEpoxyModel(
    ...
    override fun hashCode(): Int {
        ...
        keys.forEach { code = 31 * code + it.hashCode() }
...
fun ModelCollector.composableInterop(
    ...
    // Note this is done to avoid ART bug in Android 12 (background
    // https://issuetracker.google.com/issues/197818595 and
    ...
    // Note that even this manual inlining might be spoiled by R8 outlining which, if enabled might
...
/**
 * [composeEpoxyModel] can be used directly in cases where more control over the epoxy model is
 * needed. Eg. When the epoxy model needs to be modified before it's added.
 */
@Deprecated(
    ...
    replaceWith =
        ReplaceWith(
            expression =
                "composeEpoxyModel(id, *keys, modelAction = modelAction, composeFunction = composeFunction)",
...
fun composeEpoxyModel(
    ...
    return ComposeEpoxyModel(*keys, composeFunction = composeFunction).apply { id(id) }
}
 ...
 *
 * This is done to avoid ART bug in Android 12 (background
 * https://issuetracker.google.com/issues/197818595 and
 * https://github.com/airbnb/epoxy/issues/1199). The main objective is to have the creation of the
 * ComposeEpoxyModel the setting of the id and the adding to the controller in the same method. Note
 * that even with this construct this might be spoiled by R8 outlining which, if enabled might
...
inline fun <reified T : EpoxyModel<*>> EpoxyInterop(
    ...
    AndroidView(
        factory = { context -> FrameLayout(context).apply { addView((model.buildView(this))) } },
        modifier = modifier,
    ) { view ->
