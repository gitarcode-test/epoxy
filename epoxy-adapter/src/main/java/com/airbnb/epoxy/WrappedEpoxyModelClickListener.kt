...
class WrappedEpoxyModelClickListener<T : EpoxyModel<*>, V> : OnClickListener, OnLongClickListener {
    ...
    constructor(clickListener: OnModelClickListener<T, V>?) {
        requireNotNull(clickListener) { "Click listener cannot be null" }
    ...
    private fun getClickedModelInfo(view: View): ClickedModelInfo? {
        val epoxyHolder =
            ListenersUtils.getEpoxyHolderForChildView(view)
        ...
        val holderToUse =
            if (boundObject is ModelGroupHolder) {
                // For a model group the clicked view could belong to any of the nested models in
                // the group.
                // We check the viewholder of each model to see if the clicked view is in that
                // hierarchy
                // in order to figure out which model it belongs to.
                // If it doesn't match any of the nested models then it could be set by the top
                // level
                // parent model.
                boundObject.viewHolders.firstOrNull { view in it.itemView.allViewsInHierarchy }
                    ?: epoxyHolder
            } else {
                epoxyHolder
            }
        ...
        return ClickedModelInfo(holderToUse.model, adapterPosition, holderToUse.objectToBind())
    }
        ...
        get() {
            return if (this is ViewGroup) {
                children
                    .flatMap {
                        sequenceOf(it) +
                            if (it is ViewGroup) it.allViewsInHierarchy else emptySequence()
                    }
                    .plus(this)
            } else {
                sequenceOf(this)
    ...
    internal operator fun ViewGroup.iterator() =
