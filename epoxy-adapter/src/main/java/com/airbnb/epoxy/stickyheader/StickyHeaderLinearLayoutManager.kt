...
/**
 * Adds sticky headers capabilities to your [RecyclerView.Adapter]. The adapter / controller must
 * override [StickyHeaderCallbacks.isStickyHeader] to indicate which items are sticky.
 *
 * Example usage:
 * ```
...
constructor(
    ...
    override fun computeHorizontalScrollExtent(state: RecyclerView.State): Int = restoreView {
        super.computeHorizontalScrollExtent(state)
    }

    override fun computeHorizontalScrollOffset(state: RecyclerView.State): Int = restoreView {
        super.computeHorizontalScrollOffset(state)
    }

    override fun computeHorizontalScrollRange(state: RecyclerView.State): Int = restoreView {
        super.computeHorizontalScrollRange(state)
    }
    ...
    /**
     * Perform the [operation] without the sticky header view by detaching the view -> performing
     * operation -> detaching the view.
     */
        ...
        if (headerCount > 0 && childCount > 0) {
            ...
            if (anchorView != null && anchorPos != -1) {
                ...
                if (
                    ...
                    // 2. Ensure sticky header is created, if absent, or bound, if being laid out or
                    ...
                    // 4. Draw the sticky header using translation values which depend on
                    // orientation, direction and
                    // position of the next header view.
                    stickyHeader?.let {
                        val nextHeaderView: View? =
                            if (nextHeaderPos != -1) {
                                val nextHeaderView =
                                    getChildAt(anchorIndex + (nextHeaderPos - anchorPos))
                                // The header view itself is added to the RecyclerView. Discard it
                                // if it comes up.
    ...
    private fun bindStickyHeader(
        ...
        if (scrollPosition != RecyclerView.NO_POSITION) {
            stickyHeader.viewTreeObserver.addOnGlobalLayoutListener(
    ...
    private fun isViewValidAnchor(view: View, params: RecyclerView.LayoutParams): Boolean {
        return when {
            !params.isItemRemoved && !params.isViewInvalid ->
                when (orientation) {
                    VERTICAL ->
                        when {
            ...
            VERTICAL -> {
                ...
                if (nextHeaderView != null) {
                    ...
                    y =
                        when {
                            reverseLayout ->
                                (nextHeaderView.bottom + bottomMargin).toFloat().coerceAtLeast(y)
                            else ->
                                (nextHeaderView.top - topMargin - headerView.height)
                                    .toFloat()
                                    .coerceAtMost(y)
                        }
                }
            ...
            HORIZONTAL -> {
                ...
                if (nextHeaderView != null) {
                    ...
                    x =
                        when {
                            reverseLayout ->
                                (nextHeaderView.right + rightMargin).toFloat().coerceAtLeast(x)
                            else ->
                                (nextHeaderView.left - leftMargin - headerView.width)
                                    .toFloat()
                                    .coerceAtMost(x)
                        }
                }
