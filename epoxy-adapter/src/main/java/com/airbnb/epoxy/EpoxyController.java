 ...
 *
 * <p>The controller maintains a {@link androidx.recyclerview.widget.RecyclerView.Adapter} with the
 * latest models, which you can get via {@link #getAdapter()} to set on your RecyclerView.
 *
 * <p>All data change notifications are applied automatically via Epoxy's diffing algorithm. All of
 ...
 * automatically.
 *
 * <p>Once a model is created and added to the controller in {@link #buildModels()} it should be
...
public abstract class EpoxyController implements ModelCollector, StickyHeaderCallbacks {
  ...
  /**
   * This is used to track whether we are currently building models. If it is non null it means a
   * thread is in the building models method. We store the thread so we can know which one is
   * building models.
   *
   * <p>Volatile because -> write only on handler, read from any thread
   */
   ...
   *
   * <p>Volatile because -> written from the build models thread, read from the main thread.
   */
  ...
  /**
   * Posting and canceling runnables is a bit expensive - it is synchronizes and iterates the list
   * of runnables. We want clients to be able to request model builds as often as they want and have
   * it act as a no-op if one is already requested, without being a performance hit. To do that we
   * track whether we have a call to build models posted already so we can avoid canceling a current
   * call and posting it again.
   */
  @RequestedModelBuildType
  private volatile int requestedModelBuildType = RequestedModelBuildType.NONE;
   ...
   *
   * <p>The exception is that the first time this is called on a new instance of {@link
   * EpoxyController} it is run synchronously. This allows state to be restored and the initial view
   * to be draw quicker.
   *
   * <p>If you would like to be alerted when models have finished building use {@link
   * #addModelBuildListener(OnModelBuildFinishedListener)}
   */
  ...
  /**
   * Whether an update to models is currently pending. This can either be because {@link
   * #requestModelBuild()} was called, or because models are currently being built or diff on a
   * background thread.
   */
  ...
  /**
   * Add a listener that will be called every time {@link #buildModels()} has finished running and
   * changes have been dispatched to the RecyclerView.
   *
   * <p>Since buildModels can be called once for many calls to {@link #requestModelBuild()}, this is
   * called just once for each buildModels execution, not for every request.
   *
   * <p>Use this to react to changes in your models that need to happen after the RecyclerView has
  ...
  /**
   * Remove a listener added with {@link #addModelBuildListener(OnModelBuildFinishedListener)}. This
   * is safe to call from inside the callback {@link
   * OnModelBuildFinishedListener#onModelBuildFinished(DiffResult)}
   */
   ...
   *
   * <p>Using this to delay a model update may be helpful in cases where user input is causing many
   ...
   *
   * <p>If a previous request is still pending it will be removed in favor of this new delay
   *
   * <p>Any call to {@link #requestModelBuild()} will override a delayed request.
   *
   * <p>In most cases you should use {@link #requestModelBuild()} instead of this.
   *
  ...
  private final Runnable buildModelsRunnable =
      new Runnable() {
        ...
        public void run() {
          ...
          // crash the app - in which case this controller would be in an invalid state and crash
          // later
   ...
   *
   * <p>Once a model is added to the controller it should be treated as immutable and never modified
   ...
   *
   * <p>You CANNOT call this method directly. Instead, call {@link #requestModelBuild()} to have the
  ...
  public interface Interceptor {
     ...
     * experiment.
     *
     * <p>The models list must not be changed after this method returns. Doing so will throw an
     * exception.
     */
   ...
   *
   * <p>Interceptors are run on the same thread that models are built on.
   *
   ...
   *
   * <p>This is different from the number of models currently on the adapter, since models on the
   ...
   *
   * <p>There are some edge cases for handling models that are added without modification, or models
    ...
    while (modelIterator.hasNext()) {
      ...
      if (!modelIds.add(model.id())) {
        ...
        onExceptionSwallowed(
            new IllegalEpoxyUsage(
                "Two models have the same ID. ID's must be unique!"
   ...
   *
   * <p>This may be useful if your models are created via server supplied data, in which case the
   ...
   * taken to build them, the time taken to diff them, and the item change outcomes from the differ.
   * The tag of the logcat message is the class name of your EpoxyController.
   *
   * <p>This is useful to verify that models are being diffed as expected, as well as to watch for
   ...
   *
   * <p>This should only be used in debug builds to avoid a performance hit in prod.
   */
   ...
   * This is intended to be used with {@link androidx.recyclerview.widget.ItemTouchHelper} to allow
   * for efficient item dragging and rearranging. It cannot be
   *
   * <p>If you call this you MUST also update the data backing your models as necessary.
   *
   * <p>This will immediately change the model's position and notify the change to the RecyclerView.
  ...
  /**
   * An way to notify the adapter that a model has changed. This is intended to be used with {@link
   * androidx.recyclerview.widget.ItemTouchHelper} to allow revert swiping a model.
   *
   * <p>This will immediately notify the change to the RecyclerView.
   *
   ...
   *
   * <p>A common use for this is being aware of duplicates when {@link
   * #setFilterDuplicates(boolean)} is enabled.
   *
   * <p>By default the global exception handler provided by {@link
   * #setGlobalExceptionHandler(ExceptionHandler)} is called with the exception. Overriding this
   * allows you to provide your own handling for a controller.
   */
  ...
  private static ExceptionHandler globalExceptionHandler =
      new ExceptionHandler() {
        ...
        public void onException(
            @NonNull EpoxyController controller, @NonNull RuntimeException exception) {
   ...
   *
   * <p>For example, you could choose to rethrow the exception in development builds, or log them in
   * production.
   *
   * <p>A common use for this is being aware of duplicates when {@link
   * #setFilterDuplicates(boolean)} is enabled.
   *
   * <p>This callback will be used in all EpoxyController classes. If you would like specific
   * handling in a certain controller you can override {@link
   * #onExceptionSwallowed(RuntimeException)} in that controller.
   */
  ...
  public interface ExceptionHandler {
     ...
     *
     * <p>For example, you could choose to rethrow the exception in development builds, or log them
     * in production.
     *
  ...
  void onAttachedToRecyclerViewInternal(RecyclerView recyclerView) {
    ...
    if (recyclerViewAttachCount > 1) {
      MainThreadExecutor.INSTANCE.handler.postDelayed(
          new Runnable() {
            @Override
            public void run() {
              ...
              if (recyclerViewAttachCount > 1) {
                onExceptionSwallowed(
                    new IllegalStateException(
                        "This EpoxyController had its adapter added to more than one ReyclerView."
                            + " Epoxy does not support attaching an adapter to multiple"
                            + " RecyclerViews because saved state will not work properly. If you"
                            + " did not intend to attach your adapter to multiple RecyclerViews you"
                            + " may be leaking a reference to a previous RecyclerView. Make sure to"
                            + " remove the adapter from any previous RecyclerViews (eg if the"
                            + " adapter is reused in a Fragment across multiple"
                            + " onCreateView/onDestroyView cycles). See"
                            + " https://github.com/airbnb/epoxy/wiki/Avoiding-Memory-Leaks for more"
                            + " information."));
              }
            }
          },
   ...
   *     model, and was previously bound to a view. This means that {@link #buildModels()} returned
   *     a model that is different from the previouslyBoundModel and the view is being rebound to
   *     incorporate the change. You can compare this previous model with the new one to see exactly
   *     what changed.
   *     <p>The newly bound model and the previously bound model are guaranteed to have the same id,
   *     but will not necessarily be of the same type depending on your implementation of {@link
   *     #buildModels()}. With common usage patterns of Epoxy they should be the same type, and will
   *     only differ if you are using different model classes with the same id.
   *     <p>Comparing the newly bound model with the previous model allows you to be more
   *     intelligent when updating your view. This may help you optimize, or make it easier to work
   *     with animations.
   ...
   *     same view will be kept. If you are using a custom item animator then the view will be the
  ...
  protected void onViewAttachedToWindow(
      @NonNull EpoxyViewHolder holder, @NonNull EpoxyModel<?> model) {}
